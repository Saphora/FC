...
namespace FC.WebAPI.Controllers.API
{
    public class CountryController : BaseAPIController
    {
        ...
        public ServiceResponse<List<UCountry>> GetPaged(int size, int page)
        {
            ...
            return new ServiceResponse<List<UCountry>>(result, HttpStatusCode.OK, "OK");
        }
        ...
        {
            return new ServiceResponse<int>(repo.GetPagedCount<UCountry>("Countries", page, sortIndex), HttpStatusCode.OK, "OK");
        }

        [HttpOptions, HttpGet, HttpPost]
        public ServiceResponse<List<Guid?>> GetAllIds()
        {
            return new ServiceResponse<List<Guid?>>(repo.GetAllIds(), HttpStatusCode.OK, "OK");
        }

        [HttpOptions, HttpGet, HttpPost]
        public ServiceResponse<List<UCountry>> GetAll()
        {
            return new ServiceResponse<List<UCountry>>(repo.GetAll(), HttpStatusCode.OK, "OK");
        }

        [HttpOptions, HttpGet, HttpPost]
        public ServiceResponse<UCountry> GetByCode(string code)
        {
            return new ServiceResponse<UCountry>(repo.GetByCode(code), HttpStatusCode.OK, "OK");
        }

        [HttpOptions, HttpGet, HttpPost]
        public ServiceResponse<UCountry> GetByID(Guid? id)
        {
            return new ServiceResponse<UCountry>(repo.GetByID(id), HttpStatusCode.OK, "OK");
        }

        [HttpOptions, HttpGet, HttpPost]
        public ServiceResponse<UCountry> GetByCultureName(string cultureName)
        {
            return new ServiceResponse<UCountry>(repo.GetByCultureName(cultureName), HttpStatusCode.OK, "OK");
        }
